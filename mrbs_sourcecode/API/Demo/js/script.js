$(function(){
	...
	var digit_holder = clock.find('.digits'),
		digit_holder_countdown = clock_countdown.find('.digits');

	$.each(positions, function(){

		if(this == ':'){
			digit_holder.append('<div class="dots">');
			digit_holder_countdown.append('<div class="dots">');
		}
		else{
			...
			for(var i=1; i<8; i++){
				...
				pos_countdown.append('<span class="d' + i + '">');
			}

			// Set the digits as key:value pairs in the digits object
			digits[this] = pos;
			digits_countdown[this] = pos_countdown;

			// Add the digit elements to the page
			
			digit_holder.append(pos);
			digit_holder_countdown.append(pos_countdown);
		}
	...
	(function update_time(){
		...
		var remain_time = calculateRemainingTime();

		digits_countdown.h1.attr('class', digit_to_name[ Math.floor(remain_time[0]/10) ]);
		digits_countdown.h2.attr('class', digit_to_name[remain_time[0]%10]);
		digits_countdown.m1.attr('class', digit_to_name[ Math.floor(remain_time[1]/10) ]);
		digits_countdown.m2.attr('class', digit_to_name[remain_time[1]%10]);
		digits_countdown.s1.attr('class', digit_to_name[ Math.floor(remain_time[2]/10) ]);
		digits_countdown.s2.attr('class', digit_to_name[remain_time[2]%10]);
	...
	alarm_set.click(function(){
		...
		// If all check is OK, send break time to api

		$.get("GetRoomForBreaktime.php?breaktime=5", function(data, status){
        	alert("Data: " + data + "\nStatus: " + status);
   		});
	...
	dialog.on('hide',function(){
		...
		dialog.find('input').eq(0).val(remain_time[0]).end().eq(1).val(remain_time[1]).end().eq(2).val(remain_time[2]);
